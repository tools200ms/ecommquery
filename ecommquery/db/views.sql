...
CREATE VIEW obj_int_latest
    AS SELECT alli.obj_id AS obj_id, alli.prop_id AS prop_id, latesti.timestamp AS timestamp, alli.checkout_id AS checkout_id, alli.value as valuei
...
CREATE VIEW obj_text_latest
    AS SELECT allt.obj_id AS obj_id, allt.prop_id AS prop_id, latestt.timestamp AS timestamp, allt.checkout_id AS checkout_id, allt.value as valuet
...
CREATE VIEW obj_comb_latest
    AS SELECT obj_id, prop_def.spc_id, prop_def.ref_name AS ref_name, checkout_id, valuet, valuei, oc.timestamp AS timestamp
...
CREATE VIEW _obj_comb_latest
    AS SELECT obj_id, prop_id, checkout_id, valuet, valuei
