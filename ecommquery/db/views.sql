...
CREATE VIEW prop_int_latest
    AS SELECT pi.obj_id AS obj_id, pi.prop_id AS prop_id, max(timestamp) AS timestamp, c.id AS checkout_id
        ...
        WHERE c.id = pi.checkout_id
        GROUP BY obj_id, prop_id;


CREATE VIEW prop_text_latest
    AS SELECT pt.obj_id AS obj_id, pt.prop_id AS prop_id, max(timestamp) AS timestamp, c.id AS checkout_id
        FROM obj_checkout c, obj_prop_text pt
        WHERE c.id = pt.checkout_id
        GROUP BY obj_id, prop_id;
...
CREATE VIEW obj_int_latest
    AS SELECT alli.obj_id AS obj_id, alli.prop_id AS prop_id, latesti.timestamp AS timestamp, alli.checkout_id AS checkout_id, alli.value as value
        ...
        WHERE latesti.prop_id = alli.prop_id AND
                latesti.obj_id = alli.obj_id AND
                latesti.checkout_id = alli.checkout_id;
...
CREATE VIEW obj_text_latest
    AS SELECT allt.obj_id AS obj_id, allt.prop_id AS prop_id, latestt.timestamp AS timestamp, allt.checkout_id AS checkout_id, allt.value as value
        ...
        WHERE latestt.prop_id = allt.prop_id AND
                latestt.obj_id = allt.obj_id AND
